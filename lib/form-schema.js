...
export const FORM = {
  ...
  fields: [
    ...
    },
    {
      ...
      // The measurements behind the size guide on the registration page. They
      // live here, beside the sizes themselves, so the two cannot drift apart:
      // add a size to `options` above and it appears in the chart with blank
      // measurements — visibly waiting for its row — rather than silently
      // dropping out of it.
      //
      // !! UNCONFIRMED !!  These came off the old RACE SINGLET chart, which
      // showed a sleeveless garment. The 2026 race shirt is a sleeved raglan
      // tee, and this ladder is a perfectly even 16-24 / 24-32 progression —
      // which reads like a supplier's template rather than measured samples.
      // Check them against an actual shirt before the form goes live, and if
      // the tee needs a sleeve measurement too, add a third key here and a
      // matching entry in SIZE_COLUMNS in public/app.js.
      sizeChart: {
        units: 'inches',
        // Half-chest across the flat garment, and shoulder to hem.
        measurements: {
          '#14': { width: 16, length: 24 },
          '#16': { width: 17, length: 25 },
          '#18': { width: 18, length: 26 },
          'S':   { width: 19, length: 27 },
          'M':   { width: 20, length: 28 },
          'L':   { width: 21, length: 29 },
          'XL':  { width: 22, length: 30 },
          '2XL': { width: 23, length: 31 },
          '3XL': { width: 24, length: 32 },
        },
      },
    },
    {
...
export function validate(body) {
  ...
  // Only an outright refusal is refused. A submission that says nothing about
  // the waiver is a page older than the box — every tab left open across a
  // deploy is one — and rejecting those would take registration down for the
  // very people already part-way through it, with a message telling them to
  // tick something they have ticked. Those are recorded as "not recorded",
  // which is what the dashboard shows, rather than turned away.
  //
  // The gate that actually stops people is on the intro page; this is the
  // backstop for a submission that states a refusal.
  if (waiverState(body) === false) {
